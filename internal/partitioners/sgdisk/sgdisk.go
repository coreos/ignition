...
var ErrBadSgdiskOutput = errors.New("sgdisk had unexpected output")
...
// NeedsPartx returns true because sgdisk does not trigger the update of the
...
// number specified in partitionNumbers. E.g. if partitionNumbers is [1,4,5], it is expected that the sgdisk
// output was from running `sgdisk --pretend <commands> --info=1 --info=4 --info=5`. It assumes the
// partition labels are well behaved (i.e. contain no control characters). It returns a map of partition
// numbers to Output structs with the start and size information as determined by sgdisk.
// The partition numbers need to be passed in because sgdisk includes them in its output.
func (op *Operation) ParseOutput(sgdiskOut string, partitionNumbers []int) (map[int]partitioners.Output, error) {
