...
func (u Util) PerformFetch(f FetchOp) (err error) {
	path := f.Node.Path
...
func FilesystemIsEmpty(dirpath string) (isEmpty bool, err error) {
	dfd, err := os.Open(dirpath)
	if err != nil {
		return false, err
	}
	defer func() {
		err = errors.Join(err, dfd.Close())
	}()

	for {
		names, readErr := dfd.Readdirnames(1)
		if readErr == io.EOF {
			return true, nil
		} else if readErr != nil {
