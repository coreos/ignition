...
import (
	...
	"github.com/coreos/butane/config/openshift/v4_23_exp/result"
...
// ToMachineConfig4_23Unvalidated translates the config to a MachineConfig.  It also
...
func (c Config) ToMachineConfig4_23Unvalidated(options common.TranslateOptions) (result.MachineConfig, translate.TranslationSet, report.Report) {
...
// ToMachineConfig4_23 translates the config to a MachineConfig.  It returns a
// report of any errors or warnings in the source and resultant config.  If
// the report has fatal errors or it encounters other problems translating,
// an error is returned.
func (c Config) ToMachineConfig4_23(options common.TranslateOptions) (result.MachineConfig, report.Report, error) {
	cfg, r, err := cutil.Translate(c, "ToMachineConfig4_23Unvalidated", options)
...
func (c Config) ToIgn3_7Unvalidated(options common.TranslateOptions) (types.Config, translate.TranslationSet, report.Report) {
	mc, ts, r := c.ToMachineConfig4_23Unvalidated(options)
	cfg := mc.Spec.Config
...
// ToConfigBytes translates from a v4.23 Butane config to a v4.23 MachineConfig or a v3.7.0-experimental Ignition config. It returns a report of any errors or
...
func ToConfigBytes(input []byte, options common.TranslateBytesOptions) ([]byte, report.Report, error) {
	if options.Raw {
		...
		return cutil.TranslateBytesYAML(input, &Config{}, "ToMachineConfig4_23", options)
	}
}
