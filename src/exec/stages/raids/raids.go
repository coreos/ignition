...
func (s stage) Run(config config.Config) (stages.StageStatus, string) {
	if len(config.Storage.Arrays) == 0 {
		return stages.StageRunRequest, "filesystems"
	}

	// TODO(vc): inject a systemd unit for running ignition's raid_creates stage which depends on device units for all disks in config
	...
	return stages.StageScheduled, "raid_creates"
}
